...
class ReportService {
    ...
    static async getStudentRoster(filters) {
        ...
        if (filters.classId)
            where.section = { classId: filters.classId };
        ...
        const summary = {
            ...
            active: students.filter((s) => s.status === types_1.StudentStatusEnum.ACTIVE).length,
            left: students.filter((s) => s.status === types_1.StudentStatusEnum.LEFT_INSTITUTION).length,
            transferred: students.filter((s) => s.status === types_1.StudentStatusEnum.TRANSFERRED).length,
            graduated: students.filter((s) => s.status === types_1.StudentStatusEnum.GRADUATED).length,
            suspended: students.filter((s) => s.status === types_1.StudentStatusEnum.SUSPENDED).length,
        };
        return {
            summary,
            rows: students.map((s) => ({
    ...
    static async getClassWiseReport(academicYearId) {
        ...
        const classStats = classes.map((c) => {
            const sectionStats = c.sections.map((sec) => {
                const activeCount = sec.students.filter((s) => s.status === types_1.StudentStatusEnum.ACTIVE).length;
    ...
    static async getDepartmentWiseReport() {
        ...
        return departments.map((d) => {
            const activeStudents = d.students.filter((s) => s.status === types_1.StudentStatusEnum.ACTIVE).length;
            return {
                departmentId: d.id,
    ...
    static async getTransferReport(limit = 100) {
        const logs = await prisma_1.prisma.studentTransferLog.findMany({
            take: limit,
    ...
    static async getAttendanceReport(filters) {
        const slotWhere = {};
        if (filters.sectionId)
            slotWhere.sectionId = filters.sectionId;
        if (filters.classId)
            slotWhere.classId = filters.classId;
        if (filters.academicYearId)
            slotWhere.academicYearId = filters.academicYearId;
        if (filters.startDate || filters.endDate) {
            slotWhere.date = {};
            if (filters.startDate)
                slotWhere.date.gte = filters.startDate;
            if (filters.endDate)
                slotWhere.date.lte = filters.endDate;
        }
    ...
    static async getFinanceReport(filters) {
        ...
        if (filters.startDate || filters.endDate) {
            ...
            if (filters.startDate)
                paymentWhere.paymentDate.gte = new Date(filters.startDate);
            if (filters.endDate)
                paymentWhere.paymentDate.lte = new Date(filters.endDate);
        }
        if (filters.academicYearId)
            paymentWhere.academicYearId = filters.academicYearId;
        ...
        const feeWhere = {};
        if (filters.academicYearId)
            feeWhere.academicYearId = filters.academicYearId;
    ...
    static async getExaminationReport(filters) {
        const examWhere = {};
        if (filters.academicYearId)
            examWhere.academicYearId = filters.academicYearId;
        const examinations = await prisma_1.prisma.examination.findMany({
            where: examWhere,
            include: {
                resultSnapshots: {
                    select: {
    ...
    static async getStaffReport() {
        ...
        const nonFaculty = await prisma_1.prisma.user.findMany({
        ...
        const todayAttendance = await prisma_1.prisma.attendance.findMany({
            where: { date: todayStr },
    ...
    // 10. Visitor Report
    static async getVisitorReport(filters) {
        const where = {};
        if (filters.startDate || filters.endDate) {
            where.entryTime = {};
            if (filters.startDate)
                where.entryTime.gte = new Date(filters.startDate + 'T00:00:00.000Z');
            if (filters.endDate)
                where.entryTime.lte = new Date(filters.endDate + 'T23:59:59.999Z');
        }
        if (filters.purpose)
    ...
    // ===== CSV EXPORT UTILITY =====
    static exportToCSV(rows, columns) {
        if (rows.length === 0)
            return '';
