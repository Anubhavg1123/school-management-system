...
exports.StudentCaseService = void 0;
...
class StudentCaseService {
    ...
    static async createCase(payload) {
        if (!payload.studentId || !payload.title || !payload.description) {
            throw new errorHandler_1.AppError('studentId, title, and description are required.', 400, 'VALIDATION_ERROR');
        }
        const student = await prisma_1.prisma.student.findUnique({ where: { id: payload.studentId } });
        if (!student) {
            throw new errorHandler_1.AppError('Student record not found.', 404, 'NOT_FOUND');
        }
        const dateStr = new Date().toISOString().slice(0, 10).replace(/-/g, '');
        const rand = Math.floor(100 + Math.random() * 900);
        const caseNumber = `CASE-${dateStr}-${rand}`;
        const studentCase = await prisma_1.prisma.studentCase.create({
            data: {
        ...
        await prisma_1.prisma.studentCaseAction.create({
            data: {
     ...
     */
    static async getCases(requesterId, requesterRole, filters) {
        const where = {};
        if (filters?.status)
        ...
        if (requesterRole === 'STUDENT') {
            const student = await prisma_1.prisma.student.findUnique({ where: { userId: requesterId } });
            if (!student)
                return { cases: [], total: 0 };
            where.studentId = student.id;
        }
        else if (requesterRole === 'PARENT') {
            const relationships = await prisma_1.prisma.guardianStudentRelationship.findMany({
     ...
     * 3. Get Case By ID
     */
    static async getCaseById(caseId, requesterId, requesterRole) {
        const studentCase = await prisma_1.prisma.studentCase.findUnique({
            where: { id: caseId },
            include: {
                student: {
                    include: {
        ...
        if (!studentCase) {
            throw new errorHandler_1.AppError('Student case not found.', 404, 'NOT_FOUND');
        }
        // Role check
        if (requesterRole === 'STUDENT' && studentCase.student.userId !== requesterId) {
            throw new errorHandler_1.AppError('Access Denied: You cannot view this case.', 403, 'FORBIDDEN');
        }
        return studentCase;
    }
    /**
     ...
     */
    static async updateCaseStatus(caseId, updaterId, payload) {
        const studentCase = await prisma_1.prisma.studentCase.findUnique({ where: { id: caseId } });
        if (!studentCase) {
            throw new errorHandler_1.AppError('Student case not found.', 404, 'NOT_FOUND');
        }
        const updateData = { status: payload.status };
        if (payload.assignedToUserId)
            updateData.assignedToUserId = payload.assignedToUserId;
        if (payload.resolution)
            updateData.resolution = payload.resolution;
        if (['RESOLVED', 'CLOSED'].includes(payload.status))
            updateData.closedAt = new Date();
        const updated = await prisma_1.prisma.studentCase.update({
            where: { id: caseId },
            data: updateData,
            include: {
     ...
     */
    static async addCaseAction(caseId, userId, note, actionType = 'NOTE_ADDED') {
        const studentCase = await prisma_1.prisma.studentCase.findUnique({ where: { id: caseId } });
        if (!studentCase) {
            throw new errorHandler_1.AppError('Student case not found.', 404, 'NOT_FOUND');
        }
        return prisma_1.prisma.studentCaseAction.create({
            data: {
                caseId,
                actionType,
                note,
    ...
    static async getCaseStats() {
        const allCases = await prisma_1.prisma.studentCase.findMany({
...
exports.StudentCaseService = StudentCaseService;
