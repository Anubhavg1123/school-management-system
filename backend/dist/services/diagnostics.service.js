...
class DiagnosticsService {
    ...
    static async runSystemDiagnostics() {
        ...
        if (fs_1.default.existsSync(backupDir)) {
            const files = fs_1.default.readdirSync(backupDir).filter((f) => f.endsWith('.db') || f.endsWith('.sql'));
            if (files.length > 0) {
                checks.push({
    ...
    static async runDataQualityAudit() {
        ...
        const studentsWithoutGuardian = await prisma_1.prisma.student.count({
            where: {
                status: 'ACTIVE',
        ...
        const classesWithoutCoordinators = await prisma_1.prisma.class.count({
    ...
    static async recordWebhookLog(data) {
        if (data.idempotencyKey) {
            const existing = await prisma_1.prisma.webhookLog.findUnique({
        ...
        const log = await prisma_1.prisma.webhookLog.create({
            data: {
    ...
    static async getWebhookLogs(limit = 50) {
        return prisma_1.prisma.webhookLog.findMany({
            take: limit,
