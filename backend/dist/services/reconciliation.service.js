...
class ReconciliationService {
    ...
    static async getEnrollmentReconciliation() {
        const activeAcademicYear = await prisma_1.prisma.academicYear.findFirst({ where: { isCurrent: true } });
        const activeStudents = await prisma_1.prisma.student.findMany({
            where: { status: 'ACTIVE' },
            include: {
    ...
    static async getAttendanceReconciliation() {
        ...
        const slotsWithNoAttendance = await prisma_1.prisma.$queryRaw `
      SELECT 
        s.id,
        s.date,
        s.status,
    ...
    `;
        ...
        const staleScheduledSlots = await prisma_1.prisma.$queryRaw `
