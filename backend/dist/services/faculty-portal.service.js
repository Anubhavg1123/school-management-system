...
exports.FacultyPortalService = void 0;
...
class FacultyPortalService {
    ...
    static async getFacultyByUserId(userId) {
        ...
        if (!faculty) {
            throw new errorHandler_1.AppError('Faculty profile not found for authenticated user.', 404);
        }
        return faculty;
    }
    /**
     ...
     */
    static async verifyFacultySectionAccess(facultyId, sectionId) {
    ...
    static async getFacultyDashboard(userId) {
        ...
        const pendingAttendanceCount = todayAttendanceSlots.filter((s) => s.status === 'OPEN' || s.status === 'SCHEDULED').length;
        const completedAttendanceCount = todayAttendanceSlots.filter((s) => s.status === 'SUBMITTED' || s.status === 'FINALIZED').length;
     ...
     */
    static async updateFacultyProfile(userId, payload) {
        const faculty = await this.getFacultyByUserId(userId);
     ...
     */
    static async getAssignedClasses(userId) {
        const faculty = await this.getFacultyByUserId(userId);
        const subjectAssignments = await prisma.facultySubjectAssignment.findMany({
            where: { facultyId: faculty.id, status: 'ACTIVE' },
            include: {
     ...
     */
    static async getAssignedStudents(userId, sectionId) {
        const faculty = await this.getFacultyByUserId(userId);
        // Verify ownership
        const hasAccess = await this.verifyFacultySectionAccess(faculty.id, sectionId);
        if (!hasAccess) {
            throw new errorHandler_1.AppError('Access Denied: You are not assigned to teach or coordinate this class section.', 403);
        }
        ...
        return students.map((std) => {
            ...
            const presentCount = std.studentAttendances.filter((a) => a.status === 'PRESENT' || a.status === 'ACADEMIC_BYPASS').length;
     ...
     */
    static async getStudentProfileForFaculty(userId, studentId) {
        const faculty = await this.getFacultyByUserId(userId);
        ...
        if (!student) {
            throw new errorHandler_1.AppError('Student record not found.', 404);
        }
        // Verify faculty ownership
        if (student.sectionId) {
            const hasAccess = await this.verifyFacultySectionAccess(faculty.id, student.sectionId);
            if (!hasAccess) {
                throw new errorHandler_1.AppError('Access Denied: You do not have authorization to view this student profile.', 403);
            }
        }
        const totalMarked = student.studentAttendances.length;
        const presentMarked = student.studentAttendances.filter((a) => a.status === 'PRESENT' || a.status === 'ACADEMIC_BYPASS').length;
     ...
     */
    static async getFacultyTimetable(userId, dayOfWeek) {
        const faculty = await this.getFacultyByUserId(userId);
     ...
     */
    static async createAssignment(userId, payload) {
        const faculty = await this.getFacultyByUserId(userId);
        ...
        if (!hasAccess) {
            throw new errorHandler_1.AppError('Access Denied: You can only assign tasks to class sections assigned to you.', 403);
        }
     ...
     */
    static async publishAssignment(userId, assignmentId) {
        const faculty = await this.getFacultyByUserId(userId);
        ...
        if (assignment.facultyId !== faculty.id) {
            throw new errorHandler_1.AppError('Access Denied: You do not own this assignment.', 403);
        }
     ...
     */
    static async getFacultyAssignments(userId, filters) {
        const faculty = await this.getFacultyByUserId(userId);
     ...
     */
    static async requestFacultyLeave(userId, payload) {
        const faculty = await this.getFacultyByUserId(userId);
     ...
     */
    static async getFacultyLeaves(userId) {
     ...
     */
    static async requestExtraClass(userId, payload) {
        const faculty = await this.getFacultyByUserId(userId);
        // Verify ownership
        const hasAccess = await this.verifyFacultySectionAccess(faculty.id, payload.sectionId);
        if (!hasAccess) {
            throw new errorHandler_1.AppError('Access Denied: You do not teach this class section.', 403);
        }
        const activeYear = await prisma.academicYear.findFirst({ where: { isCurrent: true } });
        if (!activeYear)
            throw new errorHandler_1.AppError('No active academic year configured.', 400);
        ...
        if (facultyConflict) {
            throw new errorHandler_1.AppError('Conflict: You already have another extra class scheduled at this time.', 409);
        }
     ...
     */
    static async getFacultyExtraClasses(userId) {
        const faculty = await this.getFacultyByUserId(userId);
     ...
     */
    static async registerVehicle(userId, payload) {
        const faculty = await this.getFacultyByUserId(userId);
        const existing = await prisma.facultyVehicleRegistration.findUnique({
            where: { vehicleNumber: payload.vehicleNumber.toUpperCase() },
        });
        if (existing) {
            throw new errorHandler_1.AppError(`Vehicle number '${payload.vehicleNumber}' is already registered in the system.`, 409);
        }
     ...
     */
    static async getFacultyVehicles(userId) {
     ...
     */
    static async reviewVehicleRegistration(reviewerUserId, registrationId, payload) {
        const registration = await prisma.facultyVehicleRegistration.findUnique({
            where: { id: registrationId },
        });
        if (!registration) {
            throw new errorHandler_1.AppError('Vehicle registration record not found.', 404);
        }
     ...
     */
    static async createClassAnnouncement(userId, payload) {
        const faculty = await this.getFacultyByUserId(userId);
        const hasAccess = await this.verifyFacultySectionAccess(faculty.id, payload.sectionId);
        if (!hasAccess) {
            throw new errorHandler_1.AppError('Access Denied: You do not teach or coordinate this section.', 403);
        }
        const activeYear = await prisma.academicYear.findFirst({ where: { isCurrent: true } });
        if (!activeYear)
            throw new errorHandler_1.AppError('No active academic year configured.', 400);
     ...
     */
    static async getFacultyAnnouncements(userId, filters) {
        const faculty = await this.getFacultyByUserId(userId);
     ...
     */
    static async getFacultyNotifications(userId) {
     ...
     */
    static async getFacultyWorkload(userId) {
        const faculty = await this.getFacultyByUserId(userId);
        const subjectAssignments = await prisma.facultySubjectAssignment.findMany({
            where: { facultyId: faculty.id, status: 'ACTIVE' },
            include: { class: true, section: true, subject: true },
...
exports.FacultyPortalService = FacultyPortalService;
