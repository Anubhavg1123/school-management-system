...
class StudentAttendanceService {
    ...
    // ----------------------------------------------------
    static async generateSlotsFromTimetable(dateStr, academicYearId, actorId) {
        ...
        if (!activeYearId) {
            ...
            if (!currentYear) {
                throw new errorHandler_1.AppError('No active academic year found for attendance slot generation.', 400, 'NO_ACTIVE_ACADEMIC_YEAR');
            }
    ...
    // ----------------------------------------------------
    static async createExtraClassSlot(extraClassRequestId, actorId) {
        ...
        if (request.status !== 'APPROVED' && !allowUnapproved) {
            throw new errorHandler_1.AppError('Cannot create attendance slot for an unapproved extra class.', 400, 'UNAPPROVED_EXTRA_CLASS');
        }
    ...
    // ----------------------------------------------------
    static async getSlotDetails(slotId, userCtx) {
        ...
        if (!slot) {
            throw new errorHandler_1.AppError('Attendance slot not found.', 404, 'SLOT_NOT_FOUND');
        }
        ...
        if (userCtx.activeRole === 'FACULTY') {
            ...
            if (faculty && slot.facultyId !== faculty.id) {
                ...
                if (!substitute) {
                    throw new errorHandler_1.AppError('Authorization violation: You are not assigned to teach or substitute this class session.', 403, 'UNAUTHORIZED_SESSION');
                }
            }
        }
            ...
            if (slot.class.departmentId && slot.class.departmentId !== userCtx.departmentId) {
                throw new errorHandler_1.AppError('Department authorization violation: Cannot access attendance sessions outside your department.', 403, 'DEPARTMENT_AUTHORIZATION_VIOLATION');
            }
        }
    ...
    // ----------------------------------------------------
    static async submitStudentAttendance(data, userCtx, ipAddress) {
        const slot = await prisma.attendanceSlot.findUnique({
        ...
        if (slot.status === 'FINALIZED') {
            throw new errorHandler_1.AppError('Attendance session is already finalized and locked. Edits must use the correction workflow.', 400, 'SLOT_FINALIZED');
        }
        // Role-based authorization check
        if (userCtx.activeRole === 'FACULTY') {
            const faculty = await prisma.faculty.findUnique({ where: { userId: userCtx.id } });
            if (!faculty) {
                throw new errorHandler_1.AppError('Faculty profile not found.', 403, 'FACULTY_NOT_FOUND');
            }
            if (slot.facultyId !== faculty.id) {
                ...
                if (!substitute) {
                    throw new errorHandler_1.AppError('Unauthorized: You are not assigned to take attendance for this session.', 403, 'UNAUTHORIZED_FACULTY');
                }
            }
        }
        else if (userCtx.activeRole === 'HOD' && userCtx.departmentId) {
            if (slot.class.departmentId && slot.class.departmentId !== userCtx.departmentId) {
                throw new errorHandler_1.AppError('Department authorization violation: Cannot submit attendance for another department.', 403, 'DEPARTMENT_AUTHORIZATION_VIOLATION');
            }
        }
    ...
    // ----------------------------------------------------
    static async requestStudentAttendanceCorrection(data, actorId, ipAddress) {
        ...
        if (!stdAtt) {
            throw new errorHandler_1.AppError('Student attendance record not found.', 404, 'RECORD_NOT_FOUND');
        }
    ...
    static async reviewStudentAttendanceCorrection(data, reviewerId, reviewerRole, reviewerDeptId, ipAddress) {
        ...
        if (!correction) {
            throw new errorHandler_1.AppError('Correction request not found.', 404, 'CORRECTION_NOT_FOUND');
        }
        if (correction.status !== 'PENDING') {
            throw new errorHandler_1.AppError(`Correction request is already ${correction.status.toLowerCase()}.`, 400, 'ALREADY_REVIEWED');
        }
        ...
        if (reviewerRole === 'HOD' && reviewerDeptId) {
            if (correction.studentAttendance.attendanceSlot.class.departmentId && correction.studentAttendance.attendanceSlot.class.departmentId !== reviewerDeptId) {
                throw new errorHandler_1.AppError('Department authorization violation: Cannot review corrections outside your department.', 403, 'DEPARTMENT_AUTHORIZATION_VIOLATION');
            }
        }
    ...
    static async reviewAcademicBypass(data, reviewerId, ipAddress) {
        const bypass = await prisma.academicBypassRequest.findUnique({ where: { id: data.bypassId } });
        if (!bypass) {
            throw new errorHandler_1.AppError('Academic bypass request not found.', 404, 'BYPASS_NOT_FOUND');
        }
    ...
    // ----------------------------------------------------
    static async getStudentAttendanceHistory(studentId, academicYearId) {
        const student = await prisma.student.findUnique({
            where: { id: studentId },
            include: {
                user: true,
        ...
        const whereSlot = {};
        if (academicYearId)
            whereSlot.academicYearId = academicYearId;
    ...
    // ----------------------------------------------------
    static async getDailyUserAttendanceSummary(filters) {
    ...
    // ----------------------------------------------------
    static async getAttendanceAnomalies(filters) {
        const where = {};
        if (filters.type)
