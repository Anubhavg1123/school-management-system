...
exports.GoLiveCheckService = void 0;
const prisma_1 = require("../prisma");
const whatsapp_service_1 = require("./whatsapp.service");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
class GoLiveCheckService {
    static async runChecks() {
        ...
        // 3. Departments
        const deptCount = await prisma_1.prisma.department.count({ where: { status: 'ACTIVE' } });
        if (deptCount > 0) {
        ...
        const sectionCount = await prisma_1.prisma.section.count();
        ...
        const adminCount = await prisma_1.prisma.user.count({ where: { activeRole: 'SUPER_ADMIN', status: 'ACTIVE' } });
        if (adminCount > 0) {
        ...
        // 11. Backup Script
        const backupScriptPath = path_1.default.resolve(__dirname, '../../scripts/db-backup.ts');
        const backupExists = fs_1.default.existsSync(backupScriptPath) || fs_1.default.existsSync(backupScriptPath.replace('.ts', '.js'));
        if (backupExists) {
        ...
        // 12. Fee Structures
        const feeStructureCount = await prisma_1.prisma.feeStructure.count({ where: { status: 'ACTIVE' } });
        ...
        const subjectCount = await prisma_1.prisma.subject.count({ where: { status: 'ACTIVE' } });
        if (subjectCount > 0) {
...
exports.GoLiveCheckService = GoLiveCheckService;
