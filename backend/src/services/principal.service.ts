...
export class PrincipalService {
  ...
  static async getDashboardMetrics() {
    ...
    const totalInactiveStudents = await prisma.student.count({
    ...
    const facultyCount = await prisma.faculty.count({ where: { status: 'ACTIVE' } });
    const nonFacultyCount = await prisma.user.count({
      where: { activeRole: 'NON_FACULTY', status: 'ACTIVE' },
    });
    const hodCount = await prisma.department.count({ where: { hodUserId: { not: null } } });
    const departmentCount = await prisma.department.count({ where: { status: 'ACTIVE' } });
    ...
    const presentStudentCount = todayStudentAttendances.filter(
      (a) => a.status === 'PRESENT' || a.status === 'ACADEMIC_BYPASS'
    ).length;
    const todayStudentAttendancePercent =
      totalStudentAttCount > 0 ? Math.round((presentStudentCount / totalStudentAttCount) * 100) : 100;

    const todayStaffAttendances = await prisma.attendance.findMany({
      where: { date: todayStr },
    ...
    const lowAttendanceCount = await prisma.studentAttendance.count({
      where: { status: 'ABSENT' },
    });
    const facultyOnLeaveCount = await prisma.facultyLeave.count({
      where: {
        status: 'APPROVED',
        startDate: { lte: now },
        endDate: { gte: now },
    ...
    const pendingFacultyLeaves = await prisma.facultyLeave.count({
      where: { status: 'PENDING' },
    });

    // 5. Pending Approval Queues across Modules
    const pendingAttendanceCorrections = await prisma.studentAttendanceCorrection.count({
      where: { status: 'PENDING' },
    });
    const pendingBypassRequests = await prisma.academicBypassRequest.count({
      where: { status: 'PENDING' },
    });
    const pendingExtraClasses = await prisma.extraClassRequest.count({
      where: { status: 'PENDING' },
    });
    const pendingVehicleApprovals = await prisma.facultyVehicleRegistration.count({
      where: { status: 'PENDING' },
    });

    // 6. Visitors & Fleet
    const activeVisitors = await prisma.visitorEntryExit.count({
      where: { status: 'INSIDE_CAMPUS' },
    });
    const todayVisitors = await prisma.visitorEntryExit.count({
    ...
    const failedMessagesCount = await prisma.notificationDelivery.count({
      where: { status: 'FAILED' },
    });
  ...
  static async getExecutiveSummary() {
    ...
    return {
      ...
      communication: {
        ...
        isWhatsAppConnected: WhatsAppService.isConfigured(),
      },
    };
  }
  ...
  static async searchGlobal(query: string) {
    if (!query || query.trim().length === 0) return { students: [], faculty: [], vehicles: [], notices: [] };
   ...
   */
  static async logOverride(userId: string, action: string, entityType: string, entityId: string, reason: string, beforeState?: any, afterState?: any) {
  ...
  static async getSystemHealth() {
    ...
    const pendingJobs = await prisma.notificationDelivery.count({ where: { status: 'QUEUED' } });
    const failedJobs = await prisma.notificationDelivery.count({ where: { status: 'FAILED' } });

    return {
      database: dbStatus,
      whatsappIntegration: WhatsAppService.isConfigured() ? 'CONFIGURED' : 'UNCONFIGURED',
      queueStatus: {
        pendingJobs,
        failedJobs,
      },
