...
export class DataImportService {
   ...
   */
  static async previewStudentImport(
    csvText: string,
    filename: string,
    ...
    const previewRows: ImportPreviewResult['preview'] = [];
    let validCount = 0;
    let invalidCount = 0;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (row['admission_number'] && errors.length === 0) {
        ...
        const dupInBatch = previewRows.find(
          (r) => r.data['admission_number'] === row['admission_number'] && r.status === 'VALID'
        );
        if (dupInBatch) {
      ...
      if (row['email'] && errors.length === 0) {
        const emailExists = await prisma.user.findFirst({ where: { email: row['email'] } });
        if (emailExists) {
   ...
   */
  static async confirmStudentImport(
    importLogId: string,
    confirmedByUserId: string,
    defaultAcademicYearId?: string
  ) {
    const importLog = await prisma.dataImportLog.findUnique({
      where: { id: importLogId },
      include: { rows: { where: { status: 'PENDING' } } },
    });

    if (!importLog) throw new AppError('Import log not found.', 404, 'IMPORT_LOG_NOT_FOUND');
    if (importLog.status !== 'PREVIEW') {
      throw new AppError('Import has already been confirmed or is in an invalid state.', 400, 'IMPORT_ALREADY_PROCESSED');
    }
    if (importLog.uploadedByUserId !== confirmedByUserId) {
      throw new AppError('Only the original uploader can confirm this import.', 403, 'FORBIDDEN');
    }
    ...
    if (!academicYearId) {
      ...
      if (!activeYear) throw new AppError('No active academic year found. Please set one before importing.', 400, 'NO_ACTIVE_ACADEMIC_YEAR');
      academicYearId = activeYear.id;
    }

    let successCount = 0;
    let failedCount = 0;
    const rowResults: Array<{ rowId: string; status: string; error?: string }> = [];

    for (const row of pendingRows) {
      const data: Record<string, string> = JSON.parse(row.rowData);
      try {
      ...
      await prisma.dataImportRow.update({
        where: { id: row.id },
        data: {
          status: rowResults[rowResults.length - 1].status as any,
   ...
   * Get import history
   */
  static async getImportLogs(uploadedByUserId?: string, limit = 50) {
    const where: any = {};
