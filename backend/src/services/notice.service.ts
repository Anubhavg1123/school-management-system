...
export interface CreateNoticePayload {
  title: string;
  content: string;
  noticeType?: string; // GENERAL, ACADEMIC, HOLIDAY, EVENT, EMERGENCY, FEE, ATTENDANCE, DEPARTMENT, CLASS
  priority?: string; // LOW, MEDIUM, HIGH, URGENT
  publishDate?: string;
  expiryDate?: string;
  targetAudience: string; // ALL, STUDENTS, FACULTY, NON_FACULTY, OFFICE_STAFF, HODS, DEPARTMENT, CLASS, SECTION, SPECIFIC_USER, PARENT_GUARDIAN
  departmentId?: string;
  classId?: string;
  sectionId?: string;
  targetUserId?: string;
...
export class NoticeService {
   ...
   */
  static async createNotice(userId: string, activeRole: string, payload: CreateNoticePayload) {
    if (!payload.title || !payload.content || !payload.targetAudience) {
      throw new AppError('Notice title, content, and target audience are required.', 400);
    }

    // Role-based targeting authorization checks
    if (payload.targetAudience === 'ALL' || payload.noticeType === 'EMERGENCY') {
      if (activeRole !== UserRoleEnum.SUPER_ADMIN && activeRole !== UserRoleEnum.OFFICE_ADMIN) {
        throw new AppError(
          'Authorization violation: Only Principal / Super Admin or Office Admin can issue institution-wide or emergency notices.',
          403,
          'FORBIDDEN_AUDIENCE_TARGETING'
        );
      }
    }

    if (payload.targetAudience === 'DEPARTMENT' || payload.departmentId) {
      if (activeRole === UserRoleEnum.HOD) {
        const hodDept = await prisma.department.findFirst({ where: { hodUserId: userId } });
        if (!hodDept || (payload.departmentId && payload.departmentId !== hodDept.id)) {
          throw new AppError('Authorization violation: HOD can only publish notices within their assigned department.', 403);
        }
      }
    }

    if (payload.targetAudience === 'CLASS' || payload.classId) {
      if (activeRole === UserRoleEnum.FACULTY) {
        ...
        if (!assignment) {
          throw new AppError('Authorization violation: Faculty can only publish notices to assigned classes.', 403);
        }
      }
    }
   ...
   */
  static async estimateRecipientCount(targetAudience: string, departmentId?: string, classId?: string, sectionId?: string) {
    let count = 0;
    ...
    switch (targetAudience) {
      case 'ALL':
        ...
        break;
      case 'STUDENTS':
        count = await prisma.student.count({ where: { status: 'ACTIVE' } });
        break;
      case 'FACULTY':
        count = await prisma.faculty.count({ where: { status: 'ACTIVE' } });
        break;
      case 'NON_FACULTY':
        count = await prisma.user.count({ where: { activeRole: 'NON_FACULTY', status: 'ACTIVE' } });
        break;
      case 'HODS':
        count = await prisma.department.count({ where: { hodUserId: { not: null } } });
        break;
      case 'DEPARTMENT':
        if (departmentId) {
   ...
   */
  static async getNoticesForUser(userId: string, activeRole: string, params?: { search?: string; type?: string }) {
    const now = new Date();
    ...
    if (params?.type) where.noticeType = params.type;
    if (params?.search) {
      where.AND = [
        {
          OR: [
   ...
   */
  static async acknowledgeNotice(userId: string, noticeId: string, ipAddress?: string) {
    ...
    if (!notice.requireAcknowledgment) {
      throw new AppError('This notice does not require formal acknowledgment.', 400);
    }
  ...
  static async processScheduledAndExpiredNotices() {
    ...
    const published = await prisma.notice.updateMany({
      where: {
        status: 'SCHEDULED',
        publishDate: { lte: now },
      },
