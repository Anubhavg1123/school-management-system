...
export class WhatsAppService {
   ...
   */
  static async sendTemplateMessage(payload: WhatsAppSendTemplatePayload) {
    if (!this.isConfigured()) {
      throw new AppError(
        'WhatsApp Business API provider is not configured. Please set WHATSAPP_ACCESS_TOKEN and WHATSAPP_PHONE_NUMBER_ID in environment.',
        400,
    ...
    if (!template || !template.isActive) {
      throw new AppError(`WhatsApp template '${payload.templateCode}' not found or inactive.`, 404);
    }

    // Validate variables
    const requiredVars: string[] = JSON.parse(template.variables || '[]');
    for (const v of requiredVars) {
      if (!payload.variables[v]) {
        throw new AppError(`Missing required template variable '{{${v}}}' for template '${payload.templateCode}'.`, 400);
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new AppError(`WhatsApp provider rejected request: ${errorMsg}`, response.status, 'WHATSAPP_PROVIDER_ERROR');
      }
      ...
      throw new AppError(`WhatsApp transmission failed: ${err.message}`, 500, 'WHATSAPP_SEND_FAILED');
    }
  }
   ...
   */
  static async processWebhook(signature: string | undefined, rawBody: string, payload: any) {
    ...
    if (webhookSecret && signature) {
      ...
      const hmac1 = crypto.createHmac('sha256', webhookSecret).update(JSON.stringify(payload)).digest('hex');
      const hmac2 = crypto.createHmac('sha256', webhookSecret).update(rawBody).digest('hex');
      ...
      for (const change of changes) {
        ...
        for (const st of statuses) {
          ...
          if (existingDelivery) {
            ...
            if (statusStr === 'FAILED') updateData.failureReason = st.errors?.[0]?.title || 'Provider failed delivery';
