...
export interface SubmitMarksItem {
  studentId: string;
  obtainedTheoryMarks?: number;
  obtainedPracticalMarks?: number;
  obtainedInternalMarks?: number;
  isAbsent?: boolean;
  remarks?: string;
}
...
export class MarksService {
   ...
   */
  static async submitStudentMarksBatch(
    facultyUserId: string,
    examinationSubjectId: string,
    marks: SubmitMarksItem[],
    isDraft: boolean = true
  ) {
    const examPaper = await prisma.examinationSubject.findUnique({
      where: { id: examinationSubjectId },
      include: { examination: true, class: true, subject: true },
    });

    if (!examPaper) throw new AppError('Examination subject paper not found.', 404);
    ...
    for (const item of marks) {
      ...
      if (theory > examPaper.maxTheoryMarks) {
        throw new AppError(
          `Validation Error: Theory marks (${theory}) exceed maximum theory marks (${examPaper.maxTheoryMarks}) for student ID ${item.studentId}.`,
          400
        );
      }
      if (practical > examPaper.maxPracticalMarks) {
        throw new AppError(
          `Validation Error: Practical marks (${practical}) exceed maximum practical marks (${examPaper.maxPracticalMarks}).`,
          400
        );
      }
      if (internal > examPaper.maxInternalMarks) {
        throw new AppError(
          `Validation Error: Internal marks (${internal}) exceed maximum internal marks (${examPaper.maxInternalMarks}).`,
          400
        );
      }
      ...
      const rec = await prisma.studentMarks.upsert({
        where: {
   ...
   */
  static async verifySubjectMarks(
    reviewerUserId: string,
    examinationSubjectId: string,
    action: 'VERIFIED' | 'RETURNED_FOR_CORRECTION',
    reason?: string
  ) {
    const status = action === 'VERIFIED' ? 'VERIFIED' : 'RETURNED_FOR_CORRECTION';

    const updated = await prisma.studentMarks.updateMany({
   ...
   */
  static async requestMarksCorrection(
    requestedByUserId: string,
    studentMarksId: string,
    requestedMarks: number,
    reason: string
  ) {
    const studentMarks = await prisma.studentMarks.findUnique({
      where: { id: studentMarksId },
    });

    if (!studentMarks) throw new AppError('Student marks record not found.', 404);

    const correction = await prisma.marksCorrectionRequest.create({
      data: {
        studentMarksId,
        requestedByUserId,
   ...
   */
  static async reviewMarksCorrection(
    reviewerUserId: string,
    requestId: string,
    action: 'APPROVED' | 'REJECTED'
  ) {
    const request = await prisma.marksCorrectionRequest.findUnique({
      where: { id: requestId },
      include: { studentMarks: true },
    });

    if (!request) throw new AppError('Marks correction request not found.', 404);
