...
export class AiInsightsService {
   ...
   */
  static async processNaturalQuery(req: NaturalQueryRequest) {
    ...
    for (const pattern of injectionPatterns) {
      if (pattern.test(rawQuery)) {
        throw new AppError(
          'Security Policy Violation: Unsafe query detected. Query contains restricted administrative injection patterns.',
          400,
          'SECURITY_PROMPT_INJECTION_REJECTED'
        );
      }
    }
    ...
    if (rawQuery.includes('attendance') && (rawQuery.includes('low') || rawQuery.includes('below') || rawQuery.includes('shortage'))) {
      ...
      const studentStats: Map<string, { total: number; present: number }> = new Map();
      for (const slot of slots) {
        for (const att of slot.studentAttendances) {
          const cur = studentStats.get(att.studentId) || { total: 0, present: 0 };
          cur.total++;
          if (['PRESENT', 'ACADEMIC_BYPASS'].includes(att.status)) cur.present++;
    ...
    if (rawQuery.includes('fee') && (rawQuery.includes('outstanding') || rawQuery.includes('due') || rawQuery.includes('pending'))) {
      if (!isExecutive) {
        throw new AppError('Access Denied: Financial query requires Office Administrator or Principal role.', 403, 'FORBIDDEN');
      }
   ...
   */
  static async getAdministrativeInsights(): Promise<ExplainableInsight[]> {
    ...
    const pendingMarksCount = await prisma.studentMarks.count({
   ...
   */
  static async generateDraftNotice(payload: { topic: string; targetAudience: string; keyPoints: string[] }) {
