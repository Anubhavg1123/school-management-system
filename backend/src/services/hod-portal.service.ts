...
export class HodPortalService {
   ...
   */
  static async getHodDepartment(userId: string, requestedDeptId?: string) {
    ...
    if (!user) {
      throw new AppError('Authenticated user record not found.', 404);
    }
    ...
    if (!assignedDept && !isSuperAdmin) {
      throw new AppError('Access Denied: You are not assigned as HOD for any department.', 403);
    }

    // Super Admin override or requested department verification
    if (requestedDeptId) {
      if (!isSuperAdmin && assignedDept && assignedDept.id !== requestedDeptId) {
        throw new AppError('Access Denied: You are not authorized to manage resources outside your assigned department.', 403);
      }
      const targetDept = await prisma.department.findUnique({ where: { id: requestedDeptId } });
      if (!targetDept) throw new AppError('Requested department not found.', 404);
      return targetDept;
    }
    ...
    if (!assignedDept) {
      ...
      if (!defaultDept) throw new AppError('No active department found.', 404);
      return defaultDept;
    }
   ...
   */
  static async getHodDashboard(userId: string, requestedDeptId?: string) {
   ...
   */
  static async getDepartmentProfile(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async updateDepartmentProfile(
    userId: string,
  ...
  static async getDepartmentFaculty(
    ...
    const where: any = { departmentId: dept.id };
   ...
   */
  static async getFacultyProfileForHod(userId: string, facultyId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
    ...
    if (!faculty) {
      throw new AppError('Faculty record not found or does not belong to your department.', 404);
    }
   ...
   */
  static async assignFacultySubject(
    userId: string,
    payload: {
      facultyId: string;
      classId: string;
    ...
    if (!faculty) {
      throw new AppError('Faculty must be an active member of your department.', 400);
    }
   ...
   */
  static async getFacultyWorkloadSummary(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
    ...
    return facultyList.map((f) => {
      ...
      return {
        ...
        coordinatorSection: f.coordinatorHistories[0]
          ...
          : null,
        workloadStatus:
          weeklyPeriodCount < 10 ? 'UNDER_UTILIZED' : weeklyPeriodCount > 25 ? 'HIGH_WORKLOAD' : 'BALANCED',
      };
    });
  }
   ...
   */
  static async getDepartmentClasses(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async assignClassCoordinator(
    ...
    if (!section || section.class.departmentId !== dept.id) {
      throw new AppError('Class section not found or does not belong to your department.', 403);
    }
  ...
  static async getDepartmentStudents(
    userId: string,
    query?: { search?: string; classId?: string; sectionId?: string; page?: number; limit?: number },
    requestedDeptId?: string
  ) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
    const page = query?.page || 1;
    ...
    const where: any = { departmentId: dept.id, status: 'ACTIVE' };
  ...
  static async getLowAttendanceDashboard(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
   ...
   */
  static async getDepartmentCorrections(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async reviewCorrection(
    ...
    if (!correction || correction.studentAttendance.student.departmentId !== dept.id) {
      throw new AppError('Correction request not found or does not belong to your department.', 403);
    }
  ...
  static async getDepartmentBypasses(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async reviewBypass(
    ...
    if (!bypass || bypass.student.departmentId !== dept.id) {
      throw new AppError('Academic bypass request not found or does not belong to your department.', 403);
    }
   ...
   */
  static async getDepartmentLeaves(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async reviewFacultyLeave(
    userId: string,
    leaveId: string,
    action: 'APPROVED' | 'REJECTED',
    reviewNotes?: string,
    ...
    if (!leave || !deptFacultyUserIds.includes(leave.userId)) {
      throw new AppError('Faculty leave request not found or does not belong to your department.', 403);
    }
  ...
  static async assignSubstituteFaculty(
    ...
    if (!subFaculty) {
      throw new AppError('Substitute faculty is not active or available.', 400);
    }
    ...
    if (leaveConflict) {
      throw new AppError('Substitute faculty is on approved leave on this date.', 409);
    }
    ...
    if (subConflict) {
      throw new AppError('Substitute faculty already has another class scheduled at this time slot.', 409);
    }
   ...
   */
  static async getDepartmentExtraClasses(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async reviewExtraClass(
    ...
    if (!extra || extra.class.departmentId !== dept.id) {
      throw new AppError('Extra class request not found or does not belong to your department.', 403);
    }
   ...
   */
  static async getDepartmentTimetable(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async createTimetableEntry(
    ...
    if (facConflict) {
      ...
      throw new AppError(`Conflict: Faculty ${facName} is already assigned at this time.`, 409);
    }
    ...
    if (roomConflict) {
      ...
      throw new AppError(`Conflict: Room ${roomNum} is already booked at this time.`, 409);
    }
    ...
    if (secConflict) {
      throw new AppError('Conflict: Section already has a class scheduled at this time slot.', 409);
    }
   ...
   */
  static async updateSectionWhatsAppConfig(
    userId: string,
    sectionId: string,
    payload: { whatsAppGroupId?: string; whatsAppGroupStatus?: string },
    requestedDeptId?: string
  ) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
    ...
    if (!section || section.class.departmentId !== dept.id) {
      throw new AppError('Section not found or does not belong to your department.', 403);
    }
   ...
   */
  static async getDepartmentNotices(userId: string, requestedDeptId?: string) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async createDepartmentNotice(
    userId: string,
    payload: { title: string; content: string; targetScope?: string; classId?: string; sectionId?: string },
    requestedDeptId?: string
  ) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
  ...
  static async generateDepartmentReport(
    userId: string,
    reportType: 'FACULTY' | 'STUDENTS' | 'TIMETABLE' | 'ATTENDANCE',
    requestedDeptId?: string
  ) {
    const dept = await this.getHodDepartment(userId, requestedDeptId);
