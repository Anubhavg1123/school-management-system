...
export interface CreateVisitorEntryPayload {
  fullName: string;
  contactNumber: string;
  visitorType: string; // PARENT, GUARDIAN, FAMILY, VENDOR, DELIVERY, GUEST, CONTRACTOR, OFFICIAL, OTHER
...
export class VisitorSecurityService {
  /**
   * 1. Register Visitor & Record Campus Entry
   */
  static async createVisitorEntry(securityUserId: string, payload: CreateVisitorEntryPayload) {
    if (!payload.fullName || !payload.contactNumber || !payload.purpose || !payload.personToMeetName) {
      throw new AppError('Visitor name, contact number, purpose, and person to meet are required.', 400);
    }
    ...
    const entryLog = await prisma.visitorEntryExit.create({
      data: {
    ...
    if (payload.vehicleNumber) {
      ...
      const isReg = await prisma.vehicle.findUnique({ where: { registrationNumber: regNo } });

      await prisma.campusVehicleLog.create({
        data: {
          vehicleNumber: regNo,
   ...
   * 2. Mark Visitor Exit
   */
  static async markVisitorExit(securityUserId: string, passNumberOrId: string, remarks?: string) {
    const entry = await prisma.visitorEntryExit.findFirst({
      where: {
    ...
    if (!entry) {
      throw new AppError('Active visitor entry record not found.', 404);
    }

    if (entry.status === 'EXITED' || entry.exitTime) {
      throw new AppError(`Visitor exit already recorded at ${entry.exitTime?.toLocaleTimeString()}.`, 400);
    }

    const now = new Date();

    const updated = await prisma.visitorEntryExit.update({
      where: { id: entry.id },
      data: {
        exitTime: now,
    ...
    await prisma.campusVehicleLog.updateMany({
   ...
   */
  static async getActiveVisitors(overstayThresholdHours: number = 4) {
    const activeEntries = await prisma.visitorEntryExit.findMany({
      where: {
   ...
   */
  static async searchStudentForVisitor(query: string) {
   ...
   */
  static async verifyRegisteredVehicle(vehicleNumber: string) {
    const regNo = vehicleNumber.trim().toUpperCase();

    // Check institutional fleet
    const instVehicle = await prisma.vehicle.findUnique({
    ...
    const personalVehicle = await prisma.facultyVehicleRegistration.findUnique({
   ...
   */
  static async recordCampusVehicleEntry(securityUserId: string, payload: { vehicleNumber: string; driverOwnerName?: string; vehicleType?: string; purpose?: string }) {
    const regNo = payload.vehicleNumber.trim().toUpperCase();
    const ver = await this.verifyRegisteredVehicle(regNo);

    const log = await prisma.campusVehicleLog.create({
      data: {
        vehicleNumber: regNo,
  ...
  static async recordCampusVehicleExit(securityUserId: string, vehicleLogId: string) {
    const log = await prisma.campusVehicleLog.findUnique({ where: { id: vehicleLogId } });
    if (!log) {
      throw new AppError('Campus vehicle entry log not found.', 404);
    }

    return prisma.campusVehicleLog.update({
   ...
   */
  static async getVisitorPass(passTokenOrNumber: string) {
    const entry = await prisma.visitorEntryExit.findFirst({
      where: {
    ...
    if (!entry) {
      throw new AppError('Invalid visitor pass reference.', 404);
    }
   ...
   */
  static async searchVisitors(params?: { search?: string; type?: string; date?: string }) {
    const where: any = {};
    if (params?.type) where.visitor = { visitorType: params.type };
    if (params?.search) {
      where.OR = [
    ...
    return prisma.visitorEntryExit.findMany({
      where,
      include: {
        visitor: true,
        entrySecurity: { select: { firstName: true, lastName: true } },
